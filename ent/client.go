...
// QueryReimbursedByTransaction queries the reimbursed_by_transaction edge of a Transaction.
func (c *TransactionClient) QueryReimbursedByTransaction(t *Transaction) *TransactionQuery {
	query := (&TransactionClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := t.ID
		step := sqlgraph.NewStep(
