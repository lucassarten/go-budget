...
// QueryReimbursedByTransaction chains the current query on the "reimbursed_by_transaction" edge.
func (tq *TransactionQuery) QueryReimbursedByTransaction() *TransactionQuery {
...
// WithReimbursedByTransaction tells the query-builder to eager-load the nodes that are connected to
// the "reimbursed_by_transaction" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TransactionQuery) WithReimbursedByTransaction(opts ...func(*TransactionQuery)) *TransactionQuery {
	query := (&TransactionClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TransactionQuery) loadReimbursedByTransaction(ctx context.Context, query *TransactionQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *Transaction)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Transaction)
	for i := range nodes {
